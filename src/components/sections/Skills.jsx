...
const Skills = () => {
  ...
  const groups = [
    { id: "all", label: "All" },
    ...skillsGrouped.map((g) => ({ id: g.id, label: g.label })),
  ];

  const allItems = useMemo(
    () =>
      skillsGrouped.flatMap((g) => g.items.map((i) => ({ ...i, group: g.id }))),
    []
  );
  const itemsToShow =
    activeGroup === "all"
      ...
      <div className="container">
              ...
              >
                ...
                <div className="skill-bar">
                  <div
                    ...
                    style={{ width: `${levelToPct(item.level)}%` }}
                  />
                </div>
              </div>
            ))}
        ...
        {viewMode === "cards" && (
          <div className="skill-groups">
            {(activeGroup === "all"
              ? skillsGrouped
              : skillsGrouped.filter((g) => g.id === activeGroup)
            ).map((group) => (
              <div key={group.id} className="group-card">
                ...
                <div className="group-chips">
                  {group.items.map((s, i) => (
                    <span
                      key={`${s.name}-${i}`}
                      className={`chip chip-${s.level.toLowerCase()}`}
                      title={s.level}
                    >
                      {s.name}
                    </span>
